...
#identify filepath
file_path = r'../compiledreports.rain'
...
def compile_raindata(file_path):
    rain_files = get_rain_urls()
    ...
    for file in rain_files:
        counter += 1
        print(f"Compiling {counter} of {len(rain_files)} files.")
        response = requests.get(file, timeout = 200)
...
while True:
    recomp = input("Would you like to recompile the data (takes several minutes) (Y or N)? ").casefold()
    if recomp == 'y' or recomp == 'n':
        break

if recomp == 'y':
    compile_raindata(file_path)
...
def plot(x_values, y_values, title):
    ...
    plt.title("Oregon Statewide Rain Reporting\n" + title)
    plt.show()
